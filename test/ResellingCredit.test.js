...
contract('ResellingCredit', () => {
  ...
  describe('# Initialization', function () {
    it('[1] Should return the same amount for Token Manager\'s account balance and ResellingCredit contract initial supply', async function () {
      const expectedTokenManagerBalance = await this.resellingCreditInstance.methods.balanceOf(this.tokenManager).call({
  ...
  describe('# RSC Token Transfer', function () {
    it('[2] Should be possible to transfer 10000 RSC tokens from Token Manager to a receiver account', async function () {
      ...
      const expectedReceiverBalance = await this.resellingCreditInstance.methods.balanceOf(this.receiver).call({
        from: this.tokenManager,
      })
  ...
  describe('# Token Allowance', function () {
    it('[3] Should be possible to allow the receiver account to spend 5000 RSC tokens from Token Manager account', async function () {
      // Set token allowance.
      const transactionReceipt = await this.resellingCreditInstance.methods.increaseAllowance(
        this.receiver,
        5000,
      ).send({
        ...this.transactionParameters,
      ...
      const expectedReceiverAllowance = await this.resellingCreditInstance.methods.allowance(this.tokenManager, this.receiver).call({
        from: this.tokenManager,
      })
    ...
    it('[4] Should be possible for the receiver to spend 2500 RSC tokens from the Token Manager account', async function () {
      // Get the previous balance from receiver.
      const previousReceiverBalance = await this.resellingCreditInstance.methods.balanceOf(this.receiver).call({
        from: this.receiver,
      })

      // Get the previous balance from Token Manager.
      const previousTokenManagerBalance = await this.resellingCreditInstance.methods.balanceOf(this.tokenManager).call({
        from: this.receiver,
      })

      // Transfer from Token Manager account sending the tx with the receiver account.
      const transactionReceipt = await this.resellingCreditInstance.methods.transferFrom(
        this.tokenManager,
        this.receiver,
        2500,
      ).send({
    ...
    it('[5] Should be possible for the Token Manager to decrease by 2500 RSC tokens the receiver allowance', async function () {
      // Decrease the token allowance for the receiver.
      const transactionReceipt = await this.resellingCreditInstance.methods.decreaseAllowance(
        this.receiver,
        2500,
      ).send({
        ...this.transactionParameters,
